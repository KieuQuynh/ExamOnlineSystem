            ...
            <nav id="sidebar">
                <div class="sidebar-background ">
                    ...
                    <div class="left-menu">
                        ...
                        <a  href="startExam?userID=${sessionScope.userID}"><h2>Exams</h2></a>
                        <a  class="active" href="showAll?userID=${sessionScope.userID}"><h2>Results</h2></a>
                    </div>
                </div>
            </nav>
                        ...
                        <div class="panel" style="float: left;max-width: 900px">
                                ...
                                <table id="rounded-corner">
                                    ...
                                    <tbody>
                                        ...
                                        <c:forEach items="${requestScope.exam}" var="s">
                                            ...
                                            <tr>
                                                <td>${s.getDate()}</td>
                                                <td>${s.getCourseByExamId().getName()}</td>
                                                <td><c:out value="${s.getStartTime()}" /> -- <c:out value="${s.getEndTime()}"/></td>

                                                <c:if test="${s.getStatus() !=''}" >
                                                    <c:if test="${s.getStatus() == 'Pass'}">
                                                        <td style="background: #00cc33;color:white">${s.getStatus()}</td>
                                                    </c:if>
                                                    <c:if test="${s.getStatus() == 'Fail'}">
                                                        <td style="background: #ff3333;color:white">${s.getStatus()}</td>
                                                    </c:if>
                                                </c:if>
                                                <c:if test="${s.getStatus() == '' }" >
                                                    <td style="background: bisque ;">Not Completed</td>
                                                </c:if>
                                                <td><a href="showAll?eid=${s.getId()}">Details</a></td>
                                            </tr>
                                        </c:forEach>
                                    </tbody>
                                </table>
                            ...
                            <c:if test="${param.eid!=null}">
                                ...
                                <table id="gradient-style" >
                                    ...
                                    <c:forEach items="${requestScope.answer}" var ="s" varStatus="loop">

                                        <tr>
                                            <td rowspan="2">${loop.index +1})</td>
                                            <td colspan="2">${s.getQuestion()}</td>
                                            <td rowspan="2">${s.getStatus() }</td>
                                        </tr>
                                        <tr >
                                            <td>Your Ans:  ${s.getAnswer()}</td>
                                            <td>Correct Ans:  ${s.getCorrect_answer() }</td>
                                        </tr>
                                        <tr>
